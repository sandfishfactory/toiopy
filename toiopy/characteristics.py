...
class BatteryCharacteristic:
    ...
    def __init__(
        ...
        self.__event_emitter: ToioEventEmitter = event_emitter
...
class ConfigurationCharacteristic:
    ...
    def get_ble_protocol_version(self):
        if self.__ble_protocol_version:
            ...
            self.__characteristic.write_value(byte_data)
    ...
    def __data2result(self, data: Buffer):
        ...
        if type_data == 0x81:
            ...
            self.__event_emitter.emit("configuration:ble-protocol-version", version)
        else:
...
class LightCharacteristic:
    ...
    def turn_on_light_with_scenario(
        ...
        data: TurnOnLightWithScenarioType = self.__spec.turn_on_light_with_scenario(
        ...
        if (
            ...
            self.__timer = set_timeout(lambda: None, data.data.total_duration_ms)
    ...
    def turn_off_light(self):
        ...
        data: TurnOffLightType = self.__spec.turn_off_light()
        self.__characteristic.write_value(data.buffer.byte_data)
...
class MotorCharacteristic:
    ...
    def move(self, left: int, right: int, duration_ms: int):
        ...
        data: MoveType = self.__spec.move(left, right, duration_ms)
        self.__characteristic.write_value(data.buffer.byte_data)
    ...
    def __on_data(self, data):
        try:
            buffer = Buffer.from_data(data)
            ret: MotorResponse = self.__spec.parse(buffer)
...
class SensorCharacteristic:
    ...
    def get_slope_status(self) -> Optional[SensorTypeData]:
        parsedData: Optional[SensorType] = self.__read()
    ...
    def __read(self) -> Optional[SensorType]:
        try:
            data = self.__characteristic.read_value()
            ...
            parsed_data: SensorType = self.__spec.parse(Buffer(data))
            return parsed_data
        except Exception as e:
            print(e)
            return None
    ...
    def __on_data(self, data):
        try:
            buffer = Buffer.from_data(data)
            parsed_data: SensorType = self.__spec.parse(buffer)
...
class SoundCharacteristic:
    ...
    def play_preset_sound(self, sound_id: int):
        ...
        data: PlayPresetSoundType = self.__spec.play_preset_sound(sound_id)
        self.__characteristic.write_value(data.buffer.byte_data)
    ...
    def stop_sound(self):
        ...
        data: StopSoundType = self.__spec.stop_sound()
        self.__characteristic.write_value(data.buffer.byte_data)
